...
public class RepositorioThread extends Thread {
	...
	public void run() {
		...
		// As� puedo colgar referencias tambi�n a mis hijos del contexto
		...
		try {
			...
			// Si esto falla, el servidor de nombres quizá no esté arrancado
			Main.errorFatal("Error inicializando, buscándome. ¿Está el servidor de nombres iniciado?");
		}
		if (x != null) {
		...
		if (soyRaiz) {
			System.out.println("Este repositorio es el raíz del sistema");
			repositorioRaiz = repositorio;
			repositorioPadre = repositorio;
		} else {
			System.out.println("Este no es el repositorio raíz");

			// He de buscar el repositorio raiz
			repositorioRaiz = localizarRepositorio(cfgRaiz);
			if (repositorioRaiz == null) {
				Main.errorFatal("No se ha podido localizar el repositorio raíz!");
			}
		...
		// Hemos terminado la inicializaci�n
	...
	private IRepositorio localizarRepositorio(String nombre) {
		...
		// contexto
		// encontrar� el repositorio usando la clave
